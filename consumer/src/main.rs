...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Consumer-level state.
    //
    // This must outlive individual broker connections so that
    // reconnecting does not erase our knowledge of messages that
    ...
    loop {
        ...
        loop {
            ...
            if processed_message_ids.contains(&broker_message_id) {
                println!(
                    "Consumer detected duplicate broker message {}. Skipping processing.",
                    broker_message_id
                );
            } else {
                println!(
                ...
                // Remember this message before acknowledging it.
                processed_message_ids.insert(broker_message_id);
            }
